...
describe('MovieService', () => {
  ...
  beforeEach(async () => {
    ...
    cachaeManager = unitRef.get(CACHE_MANAGER);
  });
  ...
  describe('findRecent', () => {
    it('should return recent movies from cache', async () => {
      ...
      jest.spyOn(cachaeManager, 'get').mockResolvedValue(cachedMovies);
      const result = await movieService.findRecent();
      expect(cachaeManager.get).toHaveBeenCalledWith('MOVIE_RECENT');
  ...
  it('should fetch recent movies from the repository and cache them if not found in cache', async () => {
    ...
    jest.spyOn(cachaeManager, 'set').mockResolvedValue(null);
    jest
    ...
    expect(cachaeManager.get).toHaveBeenCalledWith('MOVIE_RECENT');
    expect(cachaeManager.set).toHaveBeenCalledWith(
      'MOVIE_RECENT',
      recentMovies,
    );
  ...
  describe('findAll', () => {
    ...
    it('should return a list of movies without user likes', async () => {
      ...
      expect(qb.andWhere).toHaveBeenCalledWith('movie.title LIKE :title', {
        title: `%${dto.title}%`,
      });
      expect(
  ...
  describe('create', () => {
    ...
    it('should create a movie and return it', async () => {
      ...
      createMovieDetailMock.mockResolvedValueOnce(movieInsertResult);
    ...
    it('should throw a NotFoundException if director does not exist', async () => {
      ...
      await expect(
        movieService.create(createMovieDto as CreateMovieDto, userId, qr),
  ...
  describe('update', () => {
    ...
    it('should update a movie successfully', async () => {
      ...
      jest
        ...
        .mockResolvedValueOnce(movie as Movie);
      (qr.manager.find as any).mockResolvedValueOnce(genres);
    ...
    it('should throw a NotFoundException if new director does not exist', async () => {
      const updateMovieDto: UpdateMovieDto = {
        title: 'Movie 2',
        directorId: 1,
      };
    ...
    it('should throw a NotFoundException if new genre does not exist', async () => {
      const updateMovieDto: UpdateMovieDto = {
        title: 'Movie 2',
        genreIds: [1, 2],
      };
